...
When ("Asesor melakukan penilaian RPL pendaftar {string}", (namaPendaftar) => {
    ...
    cy.get('#form_list > div > div > div > div.card-header > div:nth-child(5) > div.col-md-7 > div > div > table:nth-child(2)').find('tr').then(($rows) => {
      ...
      $rows.each((index, $row) => {
        // Mengabaikan baris ke-3 dan kelipatannya
        if ((index + 1) % 7 === 0) {
          cy.log(`Melewati baris ke-${index + 1}`)
          return // Melanjutkan ke baris selanjutnya
        }
        ...
        if (nameAttribute) {
          ...
          if (matches && matches.length > 1) {
            const angka = matches[1] // Mendapatkan angka yang cocok
...
When ('Asesor menambahkan {string} matakuliah yang tidak dipilih pendaftar {string}', (jumlahMK,namaPendaftar) => {
  cy.get("#sub-peminat-penilaian_rpl").click()
    cy.get('#sub-peminat-penilaian_rpl > h4 > .badge').invoke('text').then((text) => {
        cy.log("Pendaftar yang perlu dinilai: " + text)
    })
    ...
    // Memberikan asessmen RPL
    cy.get('#form_list > div > div > div > div.card-header > div:nth-child(5) > div.col-md-7 > div > div > table:nth-child(4)').find('tr').then(($rows) => {
        const numRows = $rows.slice(1).length
        cy.log(`Jumlah mata kuliah yang harus dinilai: ${numRows}`)
        ...
        $rows.each((index, $row) => {
          if (jumlahMK < 5) {
            ...
            // Apabila sudah memberikan nilai pada 3 data, hentikan proses selanjutnya
            return false
          }
        })
